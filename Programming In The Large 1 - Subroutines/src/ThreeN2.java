 ...
 * After a sequence has been displayed, the number of terms in that sequence
...
public class ThreeN2 {
    ...
    public static void main( String[] args ) {
        ...
        int K;  // Starting point for the sequence, specified by the user.
        do {
    ...
    static void print3NSequence( int startingValue ) {
        ...
        count = 1;   // We have on term so far.

        System.out.println( "The 3N + 1 sequence starting from " + N );
        System.out.println();
        System.out.printf( "%8d", N );  // Print initial term, using 8 characters.
        onLine = 1;  // There's now 1 term on the current output line.

        while ( N > 1 ) {
            N = nextN( N );  // Compute next term.
            count++;  // Count this term.
            if ( onLine == 5 ) {
                // If current output line is full, then output a carriage return
                // and note that there are no terms on the new line.
                System.out.println();
                onLine = 0;
            }
